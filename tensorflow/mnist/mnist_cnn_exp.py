...
conv1 = tf.nn.relu(conv2d(x_in, weights['conv1']) + biases['conv1'])
pool1 = maxpool2d(conv1)
norm1 = tf.nn.lrn(pool1)

conv2 = tf.nn.relu(conv2d(norm1, weights['conv2']) + biases['conv2'])
pool2 = maxpool2d(conv2)
norm2 = tf.nn.lrn(pool2)

fc1 = tf.reshape(norm2, [-1, 7*7*16])
...
with tf.Session() as sess:
	...
	for epoch in range(epochs):
		...
		# a, c = sess.run([acc, sess], feed_dict = {x: xs, y: ys})
		a = sess.run(acc, feed_dict = {x: xs, y: ys})
		print("Accuracy:", a)
	...
	save_path = saver.save(sess, "models/cnn_5/model0.ckpt")
	print("Model saved in path: %s\n" % save_path)
